...
class Ion_auth_model extends CI_Model {
	...
	public function register($identity, $password, $email, $additional_data = array(), $groups = array()) {
		...
		$query = $this->db->get_where ( $this->tables ['groups'], array (
				'name' => $this->config->item ( 'default_group', 'ion_auth' ) 
		), 1 )->row ();
	...
	public function login($identity, $password, $remember = FALSE) {
		...
		$query = $this->db->select ( $this->identity_column . ', email, id,username, password, active, last_login' )->where ( $this->identity_column, $identity )->limit ( 1 )->order_by ( 'id', 'desc' )->get ( $this->tables ['users'] );
	...
	/**
	 * Verifies if the session should be rechecked according to the configuration item recheck_timer.
	 * If it does, then
		...
		if ($recheck !== 0) {
			...
			if ($last_login + $recheck < time ()) {
				$query = $this->db->select ( 'id' )->where ( array (
						...
						'active' => '1' 
				) )->limit ( 1 )->order_by ( 'id', 'desc' )->get ( $this->tables ['users'] );
				if ($query->num_rows () === 1) {
	 ...
	 * @param string $identity
	 *        	User's identity
	 * @param int $old_attempts_expire_period
	 *        	In seconds, any attempts older than this value will be removed.
	...
	public function set_session($user) {
		...
		$group_id = $this->Users_groups_model->get_users_groups_byId ($user->id );
		$session_data ['group_id'] = $group_id->group_id;
	...
	public function login_remembered_user() {
		...
		// get the user
		$this->trigger_events ( 'extra_where' );
		$query = $this->db->select ( $this->identity_column . ', id, email, last_login' )->where ( $this->identity_column, urldecode ( get_cookie ( $this->config->item ( 'identity_cookie_name', 'ion_auth' ) ) ) )->where ( 'remember_code', get_cookie ( $this->config->item ( 'remember_cookie_name', 'ion_auth' ) ) )->where ( 'active', 1 )->limit ( 1 )->order_by ( 'id', 'desc' )->get ( $this->tables ['users'] );
	 ...
	 * @param string|bool $group_name        	
	 * @param string|array $additional_data
	 *        	IMPORTANT! This was string type $description; strings are still allowed
